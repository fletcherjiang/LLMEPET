...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--dset_name", type=str,  choices=["hl", 'tvsum', 'charadesSTA', 'tacos', 'nlq','youtube_uni', 'ego4d'])
        ...
        parser.add_argument("--main_metric", type=str, default="MR-full-mAP-key")
        ...
        #LLAMA
        parser.add_argument('--first_layer', default=7, type=int,
                            help="LLAMA Begin")
        ...
        parser.add_argument("--event_coef", default=0.1, type=float)
    ...
    def parse(self, a_feat_dir=None):
        ...
        if isinstance(self, TestOptions):
            ...
            opt.results_dir = os.path.join(opt.results_root,
                                           "-".join([opt.dset_name, ctx_str, opt.exp_id, str(opt.n_layers),str(opt.event_coef), str(opt.pos_coef)]))
            mkdirp(opt.results_dir)
            save_fns = ['llm_epet/model.py', 'llm_epet/transformer.py']
