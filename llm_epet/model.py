...
class SetCriterion(nn.Module):
    ...
    def loss_event_spans(self, outputs, targets, indices):
        ...
        loss_event_giou = 1 - torch.diag(
            generalized_temporal_iou_(span_cxw_to_xx(src_spans), span_cxw_to_xx(tgt_spans)))
        return {
    ...
    def loss_pos(self, outputs, targets, indices):
        ...
        for video_idx in range(len(src_pos)):
            ...
            for span in src_event[video_idx]:
                ...
                # target_values = src_pos[video_idx][center] 
                center, width = span.int()
                target_values = src_pos[video_idx, center, :].unsqueeze(0)  # 将其增加一个维度，使其成为[1, 256]
                # 重复这个向量以匹配 time_step_values 的行数
                if end_idx - start_idx < 0:
                    video_loss += torch.exp(step_loss)
                    continue

                target_values = target_values.repeat(end_idx - start_idx, 1)  # 现在形状为[(end_idx-start_idx), 256]
                before_center = src_pos[video_idx][start_idx:center]  # 包括 start_idx，但不包括 center
                after_center = src_pos[video_idx][center+1:end_idx+1]  # 从 center+1 到 end_idx
                time_step_values = torch.cat((before_center, after_center), dim=0)
                # time_step_values = src_pos[video_idx][start_idx:end_idx+1]
            ...
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in losses_target:
                    ...
                    if "pos" == loss:
                        continue
                    kwargs = {}
                    # aux_outputs['pseudo_event_spans'] = outputs['pseudo_event_spans']
...
def build_model(args):
    ...
    criterion = SetCriterion(
        ...
        saliency_margin=args.saliency_margin, event_matcher=event_matcher, use_matcher=use_matcher, args=args
    )
    criterion.to(device)
